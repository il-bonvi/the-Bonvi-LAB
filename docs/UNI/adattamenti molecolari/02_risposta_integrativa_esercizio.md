...
![alt text](images/2/controllo.png)
...
graph LR
    ...
    classDef corpoL1 fill:#a55edd,stroke:#8854d0,stroke-width:2px,font-weight:bold;
    classDef corpoL2 fill:#f3e8ff,stroke:#a55edd,stroke-width:1.5px;

    classDef trasdL1 fill:#ff7f50,stroke:#ff6348,stroke-width:2px,font-weight:bold;
    classDef trasdL2 fill:#ffeaa7,stroke:#fdcb6e,stroke-width:1.5px;

    classDef dopL1 fill:#2ed573,stroke:#26af58,stroke-width:2px,font-weight:bold;
    classDef dopL2 fill:#e3fcef,stroke:#a3e9c9,stroke-width:1.5px;

    Vent((Controllo della<br/>ventilazione)):::root

    Vent --> Via([Vie nervose]):::viaL1
    Via --> ViaAff([Via afferente:<br/>chemocettori, barocettori]):::viaL2
    Via --> ViaEff([Via efferente:<br/>nervo frenico, intercostali]):::viaL2

    Vent --> Corpo([Corpo carotideo/aortico]):::corpoL1
    Corpo --> CorpoI([Cellule tipo I: sensoriali]):::corpoL2
    Corpo --> CorpoII([Cellule tipo II: supporto]):::corpoL2

    Vent --> Trasd([Trasduzione del segnale]):::trasdL1
    Trasd --> TrasdK([Chiusura canali K+ →<br/>depolarizzazione]):::trasdL2
    Trasd --> TrasdNT([Rilascio ATP, ACh, GABA]):::trasdL2

    Vent --> Dop([Doping respiratorio]):::dopL1
    Dop --> DopAmi([Bloccanti canali K+:<br/>amifenazolo, niketamide]):::dopL2
    Dop --> DopPent([Pentetrazolo:<br/>inibitore GABAergico]):::dopL2
```
...
![alt text](images/2/para_simp_2.png)
...
graph LR
    ...
    classDef inputL1 fill:#74b9ff,stroke:#0984e3,stroke-width:2px,font-weight:bold;
    classDef inputL2 fill:#e0f2fe,stroke:#74b9ff,stroke-width:1.5px;

    classDef snaL1 fill:#a55edd,stroke:#8854d0,stroke-width:2px,font-weight:bold;
    classDef snaL2 fill:#f3e8ff,stroke:#a55edd,stroke-width:1.5px;

    classDef effL1 fill:#ff7f50,stroke:#ff6348,stroke-width:2px,font-weight:bold;
    classDef effL2 fill:#ffeaa7,stroke:#fdcb6e,stroke-width:1.5px;

    classDef farmL1 fill:#2ed573,stroke:#26af58,stroke-width:2px,font-weight:bold;
    classDef farmL2 fill:#e3fcef,stroke:#a3e9c9,stroke-width:1.5px;

    CO((Controllo del<br/>Cardiac Output)):::root

    CO --> Input([Input al centro CV]):::inputL1
    Input --> In1([Recettori cardiaci e vascolari]):::inputL2
    Input --> In2([Riflesso pressorio da esercizio]):::inputL2
    Input --> In3([Comando centrale]):::inputL2

    CO --> SNA([Sistema nervoso autonomo]):::snaL1
    SNA --> Simp([Simpatico ↑: NA su α/β]):::snaL2
    SNA --> Para([Parasimpatico ↓: ACh su M2]):::snaL2

    CO --> Eff([Effetti su HR e SV]):::effL1
    Eff --> EffHR([Cronotropia, dromotropia, inotropia]):::effL2
    Eff --> EffSV([Frank-Starling, ritorno venoso,<br/>pompa muscolare/respiratoria]):::effL2

    CO --> Farm([Farmaci e doping]):::farmL1
    Farm --> FarmBeta([Beta-bloccanti:<br/>↓HR, ↓contrattilità]):::farmL2
    Farm --> FarmAgo([Beta-agonisti:<br/>broncodilatazione]):::farmL2
```
...
graph LR
    ...
    classDef ctrlL1 fill:#74b9ff,stroke:#0984e3,stroke-width:2px,font-weight:bold;
    classDef ctrlL2 fill:#e0f2fe,stroke:#74b9ff,stroke-width:1.5px;

    classDef baroL1 fill:#a55edd,stroke:#8854d0,stroke-width:2px,font-weight:bold;
    classDef baroL2 fill:#f3e8ff,stroke:#a55edd,stroke-width:1.5px;

    classDef readL1 fill:#ff7f50,stroke:#ff6348,stroke-width:2px,font-weight:bold;
    classDef readL2 fill:#ffeaa7,stroke:#fdcb6e,stroke-width:1.5px;

    PA((Controllo della<br/>pressione arteriosa)):::root

    PA --> Ctrl([Regolazione riflessa]):::ctrlL1
    Ctrl --> CtrlAum([↑ MAP → ↑ parasimpatico<br/>↓ tono simpatico]):::ctrlL2
    Ctrl --> CtrlDim([↓ MAP → ↑ tono simpatico<br/>vasocostrizione]):::ctrlL2

    PA --> Baro([Barocettori]):::baroL1
    Baro --> BaroSede([Seno carotideo,<br/>arco aortico]):::baroL2
    Baro --> BaroStim([Sensibili ad allungamento<br/>assoluto e velocità]):::baroL2

    PA --> Read([Riadattamento in esercizio]):::readL1
    Read --> ReadShift([Spostamento del punto<br/>operativo, sensibilità invariata]):::readL2
    Read --> ReadMec([Inibizione GABAergica<br/>a livello dell'NTS]):::readL2
```
...
![alt text](images/2/dyna_iso.png)
