...
graph LR
    ...
    classDef trasL1 fill:#74b9ff,stroke:#0984e3,stroke-width:2px,font-weight:bold;
    classDef trasL2 fill:#e0f2fe,stroke:#74b9ff,stroke-width:1.5px;

    classDef mitoL1 fill:#a55edd,stroke:#8854d0,stroke-width:2px,font-weight:bold;
    classDef mitoL2 fill:#f3e8ff,stroke:#a55edd,stroke-width:1.5px;

    classDef recL1 fill:#ff7f50,stroke:#ff6348,stroke-width:2px,font-weight:bold;
    classDef recL2 fill:#ffeaa7,stroke:#fdcb6e,stroke-width:1.5px;

    FA((Controllo<br/>dell'ossidazione<br/>degli acidi grassi)):::root

    FA --> Tras([Trasporto in cellula]):::trasL1
    Tras --> TrasProt([FABPpm, FAT/CD36,<br/>FATP1/4]):::trasL2
    Tras --> TrasAmpk([AMPK-dipendente,<br/>AICAR-inducibile]):::trasL2

    FA --> Mito([Verso il mitocondrio]):::mitoL1
    Mito --> MitoImtg([Lipolisi IMTG:<br/>ATGL, HSL]):::mitoL2
    Mito --> MitoCpt([↓Malonil-CoA →<br/>disinibizione CPT1]):::mitoL2

    FA --> Rec([Recettori nucleari]):::recL1
    Rec --> RecPpar([PPARβ/δ: fibre ossidative]):::recL2
    Rec --> RecErr([ERRα/γ + PGC-1<br/>vs NCoR1]):::recL2
```
...
graph TD
    ...
    classDef cronL1 fill:#a55edd,stroke:#8854d0,stroke-width:2px,font-weight:bold;
    classDef cronL2 fill:#f3e8ff,stroke:#a55edd,stroke-width:1.5px;

    Glu(["<b>Controllo del glucosio</b>"]):::root
    Glu --> Acu(["Risposta acuta"]):::acuL1
    Acu --> AcuTrasl(["Traslocazione GLUT4<br/>AMPK-dipendente"]):::acuL2
    Acu --> AcuFlusso(["↑Flusso sanguigno muscolare"]):::acuL2

    Glu --> Cron(["Risposta cronica<br/>(allenamento)"]):::cronL1
    Cron --> CronGlut4(["↑Contenuto totale<br/>di GLUT4"]):::cronL2
    Cron --> CronTrasc(["Trascrizione: asse<br/>PGC-1/MEF2, NURR1, Mediator"]):::cronL2
```
