...
graph LR
    ...
    classDef ribL1 fill:#a55edd,stroke:#8854d0,stroke-width:2px,font-weight:bold;
    classDef ribL2 fill:#f3e8ff,stroke:#a55edd,stroke-width:1.5px;

    classDef conL1 fill:#ff7f50,stroke:#ff6348,stroke-width:2px,font-weight:bold;
    classDef conL2 fill:#ffeaa7,stroke:#fdcb6e,stroke-width:1.5px;

    classDef atrL1 fill:#2ed573,stroke:#26af58,stroke-width:2px,font-weight:bold;
    classDef atrL2 fill:#e3fcef,stroke:#a3e9c9,stroke-width:1.5px;

    Sig((Vie di segnalazione<br/>dell'ipertrofia)):::root

    Sig --> Via([Le tre vie principali]):::viaL1
    Via --> ViaCresc([IGF-1 → AKT → mTOR]):::viaL2
    Via --> ViaCa([Ca2+ → Calcineurina → NFAT]):::viaL2
    Via --> ViaDeg([AMPK → FOXO → Atrogin/MuRF1]):::viaL2

    Sig --> Rib([mTOR e ribosomi]):::ribL1
    Rib --> RibPol([3 RNA polimerasi<br/>coordinate da mTORC1]):::ribL2
    Rib --> RibSa([Ablazione sinergica:<br/>rapamicina inverte l'ipertrofia]):::ribL2

    Sig --> Con([Durante la contrazione]):::conL1
    Con --> ConEef2([eEF2K blocca<br/>la traduzione]):::conL2

    Sig --> Atr([Atrofia muscolare]):::atrL1
    Atr --> AtrCarico([Ipertrofia dipende<br/>dal carico meccanico]):::atrL2
```
