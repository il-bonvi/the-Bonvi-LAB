...
```mermaid
flowchart TD
    A[Pre-season: pannello completo, baseline individuale] --> B[Stagione agonistica: controlli periodici emocromo e ferro]
    B --> C[Periodi di carico elevato: monitoraggio CK, PCR, ferritina ravvicinato]
    C --> D[Taper pre-gara: controllo 2-3 settimane prima, stato marziale e infiammatorio]
    D --> E[Gara]
    E --> F[Post-gara: CK, LDH, PCR, elettroliti a seconda del timing e dell'obiettivo]
    F --> B
```
