...
```mermaid
flowchart TD
    A[4-6 settimane prima: screening completo Hb, ferritina, TSAT, PCR] --> B{Ferritina adeguata e PCR normale?}
    B -- No --> C[Supplementazione marziale mirata e/o gestione del carico infiammatorio]
    C --> D[Ricontrollo pannello marziale prima della partenza]
    B -- Si --> E[Partenza per l'altura]
    D --> E
    E --> F[Giorni 2-4 in quota: controllo reticolociti]
    F --> G{Risposta reticolocitaria adeguata?}
    G -- No --> H[Rivalutare stato marziale e infiammatorio, considerare supplementazione in quota]
    G -- Si --> I[Proseguire il protocollo di allenamento in quota]
    I --> J[Settimana 2-3: controllo Hb, Ht, reticolociti]
    J --> K[Rientro: monitoraggio Hb/Ht per la finestra di beneficio post-altura]
```
